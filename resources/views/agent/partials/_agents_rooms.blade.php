<div class="col col-sm-3 text-center">
		...
		<div class="details clearfix">
			...
			<ul class="list-unstyled">
				<li class="clearfix">
					...
					<span class="text-left" ng-bind="vm.room.user.first_name"></span>
				</li>
				<li class="clearfix">
					<span class="text-right">last name</span>
					<span class="text-left" ng-bind="vm.room.user.last_name"></span>
				</li>
				<li class="clearfix">
					<span class="text-right">horoscope sign</span>
					<span class="text-left" ng-bind="vm.room.user.sign"></span>
				</li>
				<li class="clearfix">
					<span class="text-right">date of birth</span>
					<span class="text-left" ng-bind="(vm.room.user.birth_month) + ', '+ (vm.room.user.birth_day) + ' ' + (vm.room.user.birth_year)"></span>
				</li>
				<li class="clearfix">
					<span class="text-right">gender</span>
					<span class="text-left" ng-bind="vm.room.user.sex"></span>
				</li>
				<li class="clearfix">
					<span class="text-right">time of birth</span>
					<span class="text-left" ng-bind="vm.room.user.birth_time"></span>
				</li>
				<li class="clearfix">
					<span class="text-right">state</span>
					<span class="text-left" id="state" ng-bind="vm.room.user.state"></span>
				</li>
				<li class="clearfix">
					<span class="text-right">city</span>
					<span class="text-left" id="city" ng-bind="vm.room.user.city"></span>
				</li>
				<li class="clearfix">
					<span class="text-right">package</span>
					<span class="text-left" ng-bind="vm.room.user.package"></span>
				</li>
				<span class="text-left" style="visibility: hidden" ng-bind="vm.room.user.ip"></span>
				<span class="text-left" style="visibility: hidden" id="lat" ng-bind="vm.room.user.lat"></span>
				<span class="text-left" style="visibility: hidden" id="lan" ng-bind="vm.room.user.lan"></span>
			</ul>
		</div>
		<div class="location">
	...
	<div class="content-block">
		...
		<!--
		<div>
			<button data-toggle="modal" data-target="#myModal"  class="btn btn-primary" ng-bind="(vm.room.user.sign) + ' info'"></button>
		</div>
		<br/>
		!-->
		<div class="content-block">
			...
			<div style="padding: 5px;">
				<button ng-click="popupWindow1(vm.room.user.sign)" class="btn btn-primary" >Sign Info Center</button>
			</div>

			<div style="padding: 5px;">
				<button ng-click="popupWindow2(vm.room.user.sign)"  class="btn btn-primary" >Sign Ruling Planets</button>
			</div>
		</div>
...
<div class="col col-sm-6 content-block center-block">
	<div class="chat-body">
		...
		<ul class="nav nav-tabs nav-justified" role="tablist">
                    ...
                    <li role="presentation"  ng-class="room.id == vm.room.id ? 'active' : '' "  ng-repeat="room in pagedItems[currentPage] | filter:chatActiveOrCompleted" ng-click="vm.currentPageNumber(currentPage)">
				<% %>
				<a  ui-sref="agents({room: room.id, agent_id: room.agent_id, client_id: room.user_id})" >
					<div class="adv-name" ng-bind="room.user.name"></div>

					<div class="new-messages" ng-bind="room.new_mess_count"></div>
		...
		<div class="clearfix tabs-switcher">
			<div class="col col-xs-1">
                            <a href="" class="btn btn-link" data-direction="prev" ng-click="prevPage()" ng-show="vm.rooms.length > 3">
					<i class="fa fa-chevron-left" aria-hidden="true"></i>
				</a>
			</div>
			<div class="col col-xs-10 text-center switcher-text">
				<!--<span style="color:red"  ng-if="!vm.room.customer_status" >Nobody here</span>-->

				<span ng-bind="(vm.room.user.first_name) + ' '+ (vm.room.user.last_name) + ' has JOINED the chat!' " ng-if="'Connected' == vm.room.customer_status" ></span>
				<span style="color:red" ng-bind="(vm.room.user.first_name) + ' '+ (vm.room.user.last_name) + ' has LEFT the chat!'" ng-if="'Disconnected' == vm.room.customer_status" ></span>
				<!--<span ng-bind="(vm.agent.name) + ' has joined the chat!'" ></span>-->
				<span style="color:#00AA00;" ng-bind="vm.typing" ></span>
			</div>
			<div class="col col-xs-1">
				<a href="" class="btn btn-link btn-next" data-direction="next" ng-click="nextPage()" ng-show="vm.rooms.length > 3">
		...
		<div class="tab-content">
			...
			<div role="tabpanel" class="tab-pane fade in active" id="chat-0">
                <div class="messages-container"  ng-scroll-bottom messages="vm.messages" typing="vm.typing">
                    ...
                    <div class="clearfix message" ng-repeat="message in vm.messages">
                        ...
                        <div class="col col-xs-2 text-center pad-zero-left">
							...
							<span class="name" ng-bind="(vm.room.user.first_name) + ' '+ (vm.room.user.last_name)"   ng-if="'agent' != message.sender_type" ></span>
                        </div>
				...
				<form ng-if="'Connected' == vm.room.customer_status" ng-keypress="typingAgent()"  ng-submit="vm.sendMessage()">
					<div class="form-group">
						<div class="input-group">
							<input ng-model="vm.message" id="em_text" onfocus="
							if(typeof $('.emoji-wysiwyg-editor').val() === 'undefined'){
					 		...
					 		" class="form-control input-lg" type="text">
                               <span class="input-group-btn">
                                 ...
                                 <button class="btn btn-default" id="emojiable-option"
										 onfocus="
                                 if(typeof $('.emoji-wysiwyg-editor').val() === 'undefined'){
									$('#em_text').emojiarea({button: '#emojiable-option'});
