<div class="breadcrumbs">
    <ul class="list-unstyled list-inline">
        <li><a id="<?php  if (isset(Auth::user()->paymentInfo)) {
                echo '';
            } else {
                echo 'add_payment_info';
            } ?>"
               href="<?php  if (isset(Auth::user()->paymentInfo)) {
                   echo '/dashboard/end-chat/<% vm.customer.agent.id %>/<% vm.customer.id %>';
               } else {
                   echo '';
               } ?>"
               onclick="<?php  if (isset(Auth::user()->paymentInfo)) {
                   echo 'event.preventDefault();	var that = this; $(\'#end-chat-modal-id\').modal();';
               } else {
                   echo '';
               } ?>"
            ><i class="fa fa-chevron-left" aria-hidden="true"></i>
                <span class="text-center"><%vm.customer.agent.name%></span>
                <img ng-src="/uploads/<%vm.customer.agent.image%>">
            </a></li>
    </ul>
</div>

<ul class="nav navbar-nav navbar-nav-controls text-center">
    <li class="col col-xs-6">
        <a href="" id="<?php  if (isset(Auth::user()->paymentInfo)) {
            echo 'email_chat';
        } else {
            echo 'add_payment_info';
        } ?>" ng-attr-data-id="<%vm.currentRoom.id%>" onclick="">
            <img height="52" src="/images/email-chat.png" alt="">
            <span>Email chat</span>
        </a>
    </li>
    <li class="col col-xs-6">
        <a class="end-chat" href="/dashboard/end-chat/<% vm.customer.agent.id %>/<% vm.customer.id %>" onclick="
								event.preventDefault();
								$('#end-chat-modal-id').modal();"
           id="end_chat">
            <img height="52" src="/images/end-chat.png" alt="">
            <span>End chat</span>
        </a>
    </li>
</ul>
...
<div class="chat-details">
    <div class="time-left"><i class="fa fa-clock-o" aria-hidden="true"></i>

        <timer id="tim" start-time="vm.currentRoom.started_chat_on" autostart="false" interval="1000"><% hhours %>:<%mminutes%>:<%sseconds%>
        </timer>
    </div>
    <div class="package-type">Package <span><?php
            ...
            <small>Min</small></span></div>
    <div>
        <a id="<?php  if (isset(Auth::user()->paymentInfo)) {
            echo '';
        } else {
            echo 'add_payment_info';
        } ?>"
           onclick="
           <?php  if (isset(Auth::user()->paymentInfo)) {
...
<div class="messages-container" ng-scroll-bottom messages="vm.messages" typing="vm.typing">
    <div class="clearfix" ng-repeat="message in vm.messages ">
        <div ng-class="message.sender_type !='agent' ?  'message advisor-message pull-right' : 'message member-message pull-left' "
             ng-bind-html="message.message | emoji"></div>
    </div>
...
<div class="chat-container">
    <form ng-if="vm.customerConnected" ng-submit="vm.sendMessage()" class="form-horizontal live-chat-form" style="">
        <div class="clearfix">

            <div class="col col-xs-9 no-padding-right">
                <input autocomplete="off" ng-keypress="typingUser()" type="text" id="messag"
                       name="{{uniqid("inputext")}}"
                       ng-model="vm.message" class="form-control input-lg">
            </div>
            <div class="col col-xs-3">
                <button type="submit" class="btn btn-green btn-lg btn-success btn-block btn-sent">Send</button>
            </div>
        </div>
    </form>
</div>
            ...
            <div class="modal-body">
                ...
                <div class="row">
                    <div class="col col-xs-5">
                        <a href="" class="btn btn-default btn-lg btn-block btn-grey ng-scope" ng-click="handleClick()"
                           id="end_yes">Yes, I am sure</a>
                    </div>
                    <div class="col col-xs-7">
                        <a href="" class="btn btn-primary btn-lg btn-block btn-blue" data-dismiss="modal" id="end_no">No,
                            take me back to chat</a>
                    </div>
                </div>
            </div>
        </div>
...
<script type="text/javascript">
    function setChatHeight() {
        var msgHeight = $(window).height() - $('header').outerHeight(true) - $('.chat-details').outerHeight(true) - $('.breadcrumbs').outerHeight(true) - $('.navbar-nav-controls').outerHeight(true) - 66;
        var setTop = $('header').outerHeight(true) + $('.chat-details').outerHeight(true) + $('.breadcrumbs').outerHeight(true) + $('.navbar-nav-controls').outerHeight(true);
...
<script type="text/ng-template" id="modal.html">
    <div class="modal" id="conn_mod" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true"
         data-backdrop="static" data-keyboard="false">
        <div class="modal-dialog" role="document">
            <div class="modal-content text-center">
                <div class="modal-header">
                    <button type="button" class="close" ng-click="close(false)" id="close_waiting" data-dismiss="modal"
                            aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
...
<script type="text/ng-template" id="20SecondsLeftModal.html">
    <div class="modal fade" id="add-minutes-modal-id-20" tabindex="-1" role="dialog"
         aria-labelledby="add-minutes-modal-label">
        <div class="modal-dialog" role="document">
            <div class="modal-content text-center">
                <div class="modal-header">
